from ai.discussion import ChatbotMessage
...
class TelegramChatbotMessage(ChatbotMessage[TelegramChatbotSender], _abc.ABC):
    @classmethod
    @_abc.abstractmethod
    ...
    @classmethod
    async def create_with_extras(cls, message: _telegram.Message, specs: ChatbotSpecs, extras, directory: _saves.ResourcesDirectory) -> _T.Self:
        return cls(message, directory)
    
    @classmethod
    async def create_from_telegram(cls, message: _telegram.Message, specs: ChatbotSpecs, creators: CreatorsMap, creators_state: CreatorsState, directory: _saves.ResourcesDirectory) -> _T.Self:
        return cls(message, directory)
    ...
    @classmethod
    def get_bot_tg_configuration(cls, specs: ChatbotSpecs) -> _saves.ResourcesDirectory:
        return specs.directory.get_directory('telegram').get_directory('conf').get_directory(cls.class_get_messages_typename())
    
    @_abc.abstractmethod
    def export_data_to_llm(self, specs: ChatbotSpecs, images: list[_local_utils_images.Image]) -> _T.Any:
        ...
